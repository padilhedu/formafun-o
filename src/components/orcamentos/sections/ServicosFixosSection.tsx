...
export function ServicosFixosSection({ itens, travado, onAdd, onRemove }: Props) {
  ...
  return (
    <div>
      ...
      <div style={cardStyle}>
        <div style={{ padding: '11px 16px', borderBottom: '1px solid rgba(0,0,0,0.07)', background: 'rgba(255,255,255,0.02)' }}>
              ...
              return (
                <tr key={proc.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', background: noOrc ? 'rgba(31,122,77,0.04)' : 'transparent' }}>
                  <td style={{ padding: '9px 12px', fontSize: 13, color: '#1C1C1C', fontFamily: 'var(--font-montserrat)' }}>
                    {proc.nome}
                    {noOrc && <span style={{ marginLeft: 8, fontSize: 10, color: '#1F7A4D', fontWeight: 700 }}>✓ incluído</span>}
                  </td>
                    ...
                    <td style={{ padding: '9px 12px', textAlign: 'center' }}>
                      {noOrc ? (
                        <button
                          ...
                          style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', color: '#C0392B', borderRadius: 6, padding: '4px 12px', fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-montserrat)' }}
