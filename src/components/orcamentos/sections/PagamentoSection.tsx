...
export function PagamentoSection({ modelos, modeloId, total, totalComDesconto, parcelasEditadas, clausulas, obsInterna, observacoes, parcelas, travado, orcamentoId, status, onModeloId, onParcelasEditadas, onClausulas, onObsInterna, onObservacoes, labelStyle }: Props) {
  ...
  const cardStyle: React.CSSProperties = {
    background: '#121214', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '18px 20px',
  };
      ...
      <div className="space-y-5">
        ...
        {modelo && modelo.desconto_avista_percentual > 0 && (
          <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(74,222,128,0.05)', border: '1px solid rgba(74,222,128,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 20, color: '#4ADE80', fontFamily: 'var(--font-montserrat)', fontWeight: 700 }}>{formatBRL(totalComDesconto)}</span>
          </div>
        )}

        {/* Tabela de parcelamento preview */}
        {parcelasEditadas.length > 0 && parcelas.length === 0 && (
          <div style={{ background: '#1A1A1E', borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 11, color: '#8A8A93', fontFamily: 'var(--font-montserrat)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              ...
              <tbody>
                {parcelasEditadas.map((p, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ padding: '7px 12px', fontSize: 12, color: '#8A8A93', fontFamily: 'var(--font-montserrat)' }}>
                      {p.n === 0 ? 'Entrada' : `${p.n}ª`}
                    </td>
                    <td style={{ padding: '7px 12px' }}>
                      {!travado ? (
                        <input
                          ...
                          style={{ background: 'transparent', border: 'none', outline: 'none', color: '#F5F2EA', fontSize: 12, fontFamily: 'var(--font-montserrat)', cursor: 'pointer' }}
                          aria-label={`Vencimento da parcela ${p.n === 0 ? 'entrada' : p.n}`}
                        />
                      ) : (
                        <span style={{ fontSize: 12, color: '#F5F2EA', fontFamily: 'var(--font-montserrat)' }}>
                          {new Date(p.vencimento + 'T12:00:00').toLocaleDateString('pt-BR')}
                        </span>
                      )}
                    </td>
                    <td style={{ padding: '7px 12px', textAlign: 'right', fontFamily: 'var(--font-montserrat)', fontSize: 13, color: '#B89A5A', fontWeight: 600 }}>
                      {formatBRL(p.valor)}
                    </td>
                  </tr>
                ))}
        ...
        {parcelas.length > 0 && (
          <div style={{ background: '#1A1A1E', borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ fontSize: 11, color: '#4ADE80', fontFamily: 'var(--font-montserrat)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                ✓ Cobranças geradas
              </span>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {parcelas.map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ padding: '8px 12px', fontSize: 12, color: '#F5F2EA', fontFamily: 'var(--font-montserrat)' }}>{p.descricao}</td>
                    <td style={{ padding: '8px 12px', fontSize: 12, color: '#8A8A93', fontFamily: 'var(--font-montserrat)' }}>
                      {new Date(p.vencimento + 'T12:00:00').toLocaleDateString('pt-BR')}
                    </td>
                    <td style={{ padding: '8px 12px', textAlign: 'right', fontSize: 13, color: '#B89A5A', fontFamily: 'var(--font-montserrat)', fontWeight: 600 }}>
                      {formatBRL(p.valor)}
                    </td>
                    <td style={{ padding: '8px 12px', textAlign: 'right', fontSize: 11, color: p.status === 'pago' ? '#4ADE80' : p.status === 'vencido' ? '#F87171' : '#FBBF24', fontFamily: 'var(--font-montserrat)', fontWeight: 600, textTransform: 'uppercase' }}>
                      {p.status}
                    </td>
                  </tr>
                ))}
