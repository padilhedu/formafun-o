...
export function AbaEvolucoes({ pacienteId, evolucoes: iniciais }: Props) {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {showForm && (
        <div style={{ borderRadius: 14, background: '#FFFFFF', border: '1px solid rgba(31,122,77,0.2)', padding: '20px', marginBottom: 20 }}>
      ...
      {evolucoes.length === 0 ? (
        ...
        <div className="relative">
          <div className="absolute left-5 top-0 bottom-0 w-px" style={{ background: 'rgba(0,0,0,0.07)' }} />
          <div className="space-y-3 pl-12">
            {evolucoes.map(ev => (
              <div key={ev.id} className="relative">
                <div className="absolute -left-7 top-4 w-2.5 h-2.5 rounded-full border-2 flex-shrink-0"
                  style={{
                    background: ev.travada ? '#FAF8F4' : '#1F7A4D',
                    borderColor: ev.travada ? '#6B6B66' : '#1F7A4D',
                  }} />
                <div style={{
                  borderRadius: 12,
                  background: '#FFFFFF',
                  border: `1px solid ${ev.travada ? 'rgba(0,0,0,0.07)' : 'rgba(74,222,128,0.12)'}`,
                  padding: '16px',
                }}>
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {ev.travada && (
                        <span className="badge" style={{ background: 'rgba(74,222,128,0.08)', color: '#1F7A4D', border: '1px solid rgba(74,222,128,0.2)', fontSize: 10 }}>
                          ✓ CFO
                        </span>
                      )}
                    </div>
                  ...
                  {ev.travada && (
                    <p style={{ fontSize: 11, marginTop: 8, color: '#1F7A4D', opacity: 0.6, fontStyle: 'italic' }}>
