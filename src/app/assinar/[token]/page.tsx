...
export default async function AssinarPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (c.status === 'assinado') {
    ...
    return <ErroPagina titulo="Contrato já assinado" mensagem={`Este contrato já foi assinado em ${assinadoStr}. Você receberá o PDF por e-mail.`} cor="#1F7A4D" />;
  }

  if (c.status === 'recusado') {
    return <ErroPagina titulo="Contrato recusado" mensagem="Este contrato foi recusado. Entre em contato com a clínica para mais informações." cor="#C0392B" />;
  }
...
function ErroPagina({ titulo, mensagem, cor = '#C0392B' }: { titulo: string; mensagem: string; cor?: string }) {
  const icon = cor === '#1F7A4D'
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F3EF', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 420, background: '#FFFFFF', borderRadius: 16, padding: 32, border: `1px solid ${cor}40`, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: '#6B6B66', fontFamily: 'var(--font-montserrat,sans-serif)', lineHeight: 1.6, margin: 0 }}>{mensagem}</p>
      </div>
    </div>
  );
}
