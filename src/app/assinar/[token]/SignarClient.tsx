...
export function AssinarClient({ token, contratoId, contratoHtml: rawHtml, pacienteNome, pacienteEmail, clinicaNome, clinicaTelefone }: Props) {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    background: 'rgba(255,255,255,0.04)', color: '#F5F2EA', fontSize: 16,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0B', color: '#F5F2EA', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 0 60px' }}>
      {/* Topbar */}
      <div style={{ width: '100%', background: '#121214', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '14px 20px', textAlign: 'center' }}>
        <p style={{ margin: 0, fontFamily: 'var(--font-cormorant,serif)', fontSize: 18, color: '#B89A5A', fontWeight: 600 }}>{clinicaNome}</p>
        <p style={{ margin: '2px 0 0', fontSize: 11, color: '#8A8A93', fontFamily: 'var(--font-montserrat,sans-serif)' }}>Assinatura Eletrônica de Contrato</p>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, margin: '24px 0 0', width: '100%', maxWidth: 480, padding: '0 20px', boxSizing: 'border-box' }}>
        {STEPS.map((label, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            ...
            {i < STEPS.length - 1 && <div style={{ flex: 1, height: 1, background: i < step ? '#B89A5A' : 'rgba(138,138,147,0.2)', margin: '0 4px', marginBottom: 20 }} />}
          </div>
        ))}
      </div>
      ...
      <div style={{ width: '100%', maxWidth: 480, padding: '0 20px', boxSizing: 'border-box', marginTop: 24 }}>
        ...
        {step === 0 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-cormorant,serif)', fontSize: 22, color: '#B89A5A', margin: '0 0 16px' }}>Leia o contrato completo</h2>
            <div
            ...
            <button
              ...
              style={{ width: '100%', marginTop: 16, padding: '14px', borderRadius: 10, background: scrolledToEnd ? '#B89A5A' : 'rgba(184,154,90,0.3)', color: scrolledToEnd ? '#0A0A0B' : '#8A8A93', fontWeight: 700, fontSize: 15, border: 'none', cursor: scrolledToEnd ? 'pointer' : 'not-allowed', fontFamily: 'var(--font-montserrat,sans-serif)', transition: 'all .2s' }}
            >
        ...
        {step === 1 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-cormorant,serif)', fontSize: 22, color: '#B89A5A', margin: '0 0 20px' }}>Seus dados</h2>
            ...
            <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer', marginBottom: 20 }}>
              <input type="checkbox" checked={declaracao} onChange={e => { setDeclaracao(e.target.checked); requestGeo(); }} style={{ width: 18, height: 18, marginTop: 2, accentColor: '#B89A5A', flexShrink: 0 }} />
            ...
            <button
              ...
              style={{ width: '100%', padding: '14px', borderRadius: 10, background: '#B89A5A', color: '#0A0A0B', fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
              Continuar →
            </button>
          </div>
        ...
        {step === 2 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-cormorant,serif)', fontSize: 22, color: '#B89A5A', margin: '0 0 8px' }}>Assine abaixo</h2>
            <p style={{ fontSize: 13, color: '#8A8A93', fontFamily: 'var(--font-montserrat,sans-serif)', marginBottom: 16 }}>
              Assine acima com o dedo ou mouse
            </p>
            <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', border: '2px solid rgba(184,154,90,0.4)' }}>
              <canvas
                ref={canvasRef}
            ...
            <button
              ...
              style={{ marginTop: 8, padding: '8px 16px', borderRadius: 8, background: 'transparent', color: '#8A8A93', border: '1px solid rgba(138,138,147,0.3)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
              Limpar
            </button>
            {submitError && (
            ...
            <button
              ...
              disabled={submitting}
              style={{ width: '100%', marginTop: 16, padding: '15px', borderRadius: 10, background: submitting ? 'rgba(184,154,90,0.5)' : '#B89A5A', color: '#0A0A0B', fontWeight: 700, fontSize: 16, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 50 }}>
          <div style={{ background: '#121214', borderRadius: 16, padding: 24, width: '100%', maxWidth: 440, border: '1px solid rgba(248,113,113,0.3)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button onClick={() => setRecusaOpen(false)} style={{ flex: 1, padding: 12, borderRadius: 8, background: 'transparent', color: '#8A8A93', border: '1px solid rgba(138,138,147,0.3)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--font-montserrat,sans-serif)' }}>
                Cancelar
              </button>
              <button
      ...
      <div style={{ marginTop: 40, textAlign: 'center' }}>
        <p style={{ fontSize: 11, color: '#8A8A93', fontFamily: 'var(--font-montserrat,sans-serif)' }}>
