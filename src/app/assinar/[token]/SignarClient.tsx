...
export function AssinarClient({ token, contratoId, contratoHtml: rawHtml, pacienteNome, pacienteEmail, clinicaNome, clinicaTelefone }: Props) {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    background: 'rgba(0,0,0,0.05)', color: '#1C1C1C', fontSize: 16,
    fontFamily: 'var(--font-montserrat,sans-serif)', outline: 'none', boxSizing: 'border-box',
  };
  const labelStyle: React.CSSProperties = { fontSize: 12, color: '#6B6B66', fontFamily: 'var(--font-montserrat,sans-serif)', marginBottom: 6, display: 'block', fontWeight: 500 };
  const errStyle: React.CSSProperties = { color: '#C0392B', fontSize: 12, marginTop: 4, fontFamily: 'var(--font-montserrat,sans-serif)' };

  return (
    <div style={{ minHeight: '100vh', background: '#F5F3EF', color: '#1C1C1C', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 0 60px' }}>
      {/* Topbar */}
      <div style={{ width: '100%', background: '#FFFFFF', borderBottom: '1px solid rgba(0,0,0,0.08)', padding: '14px 20px', textAlign: 'center' }}>
        ...
        {STEPS.map((label, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 60 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-montserrat,sans-serif)', background: i <= step ? '#1F7A4D' : 'rgba(138,138,147,0.2)', color: i <= step ? '#F5F3EF' : '#6B6B66', border: i === step ? '2px solid #6B6B66' : 'none', transition: 'all .3s' }}>
                {i < step
                  ? <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="#F5F3EF" strokeWidth="2"><path d="M2 6.5l3 3 6-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  : i + 1}
              </div>
              <span style={{ fontSize: 10, marginTop: 4, color: i === step ? '#6B6B66' : '#6B6B66', fontFamily: 'var(--font-montserrat,sans-serif)', textAlign: 'center' }}>{label}</span>
            </div>
      ...
      <div style={{ width: '100%', maxWidth: 480, padding: '0 20px', boxSizing: 'border-box', marginTop: 24 }}>
        ...
        {step === 0 && (
          <div>
            ...
            {!scrolledToEnd && (
              <p style={{ fontSize: 12, color: '#C98A1E', textAlign: 'center', marginTop: 10, fontFamily: 'var(--font-montserrat,sans-serif)' }}>
            ...
            <button
              ...
              style={{ width: '100%', marginTop: 16, padding: '14px', borderRadius: 10, background: scrolledToEnd ? '#1F7A4D' : 'rgba(31,122,77,0.3)', color: scrolledToEnd ? '#F5F3EF' : '#6B6B66', fontWeight: 700, fontSize: 15, border: 'none', cursor: scrolledToEnd ? 'pointer' : 'not-allowed', fontFamily: 'var(--font-montserrat,sans-serif)', transition: 'all .2s' }}
            >
              Li e aceito o contrato
            </button>
            <button
              onClick={() => setRecusaOpen(true)}
              style={{ width: '100%', marginTop: 10, padding: '12px', borderRadius: 10, background: 'transparent', color: '#C0392B', fontWeight: 500, fontSize: 13, border: '1px solid rgba(248,113,113,0.3)', cursor: 'pointer', fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
        ...
        {step === 1 && (
          <div>
            ...
            <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer', marginBottom: 20 }}>
              ...
              <span style={{ fontSize: 13, color: '#6B6B66', fontFamily: 'var(--font-montserrat,sans-serif)', lineHeight: 1.5 }}>
            ...
            <button
              ...
              style={{ width: '100%', padding: '14px', borderRadius: 10, background: '#1F7A4D', color: '#F5F3EF', fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
              Continuar →
            </button>
          </div>
        ...
        {step === 2 && (
          <div>
            ...
            {submitError && (
              <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 8, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#C0392B', fontSize: 13, fontFamily: 'var(--font-montserrat,sans-serif)' }}>
                {submitError}
              </div>
            )}
            <button
              ...
              disabled={submitting}
              style={{ width: '100%', marginTop: 16, padding: '15px', borderRadius: 10, background: submitting ? 'rgba(31,122,77,0.5)' : '#1F7A4D', color: '#F5F3EF', fontWeight: 700, fontSize: 16, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-montserrat,sans-serif)' }}
            >
      ...
      {recusaOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 50 }}>
          <div style={{ background: '#FFFFFF', borderRadius: 16, padding: 24, width: '100%', maxWidth: 440, border: '1px solid rgba(248,113,113,0.3)' }}>
            <h3 style={{ fontFamily: 'var(--font-cormorant,serif)', fontSize: 20, color: '#C0392B', margin: '0 0 12px' }}>Recusar contrato</h3>
            <p style={{ fontSize: 13, color: '#6B6B66', fontFamily: 'var(--font-montserrat,sans-serif)', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              ...
              <button
                ...
                style={{ flex: 1, padding: 12, borderRadius: 8, background: 'rgba(248,113,113,0.15)', color: '#C0392B', border: '1px solid rgba(248,113,113,0.3)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--font-montserrat,sans-serif)', fontWeight: 600 }}
              >
