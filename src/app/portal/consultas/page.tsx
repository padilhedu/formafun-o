...
const STATUS_LABEL: Record<string, { label: string; color: string }> = {
  ...
  confirmado: { label: 'Confirmado', color: '#4ADE80' },
  realizado:  { label: 'Realizado', color: '#8A8A93' },
  cancelado:  { label: 'Cancelado', color: '#F87171' },
  faltou:     { label: 'Faltou', color: '#FBBF24' },
};
...
export default async function PortalConsultasPage() {
      ...
      <div className="card" style={{ padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#8A8A93', marginTop: 3 }}>
              {new Date(c!.data_hora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
              {c!.duracao_minutos && ` · ${c!.duracao_minutos} min`}
              {c!.tipo && ` · ${c!.tipo}`}
            </p>
            {c!.notas_publicas && <p style={{ fontSize: 12, color: '#8A8A93', marginTop: 4 }}>{c!.notas_publicas}</p>}
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {(futuras ?? []).length > 0 && (
        <div>
          <p style={{ fontSize: 11, color: '#B89A5A', fontFamily: 'var(--font-montserrat)', fontWeight: 700, marginBottom: 10 }}>Próximas</p>
      ...
      {(passadas ?? []).length > 0 && (
        <div>
          <p style={{ fontSize: 11, color: '#8A8A93', fontFamily: 'var(--font-montserrat)', fontWeight: 700, marginBottom: 10 }}>Histórico</p>
      ...
      {(futuras ?? []).length === 0 && (passadas ?? []).length === 0 && (
        <div className="card p-8 text-center">
          <p style={{ color: '#8A8A93', fontSize: 13 }}>Nenhuma consulta registrada.</p>
        </div>
      )}
    </div>
  );
